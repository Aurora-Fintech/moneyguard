...
async function fetchFromFrankfurter(days) {
  ...
  if (!usdData.rates || !eurData.rates) throw new Error("Frankfurter invalid payload");

  const dates = Object.keys(usdData.rates).filter((d) => eurData.rates[d]).sort();
...
async function fetchFromExchangeHost(days) {
  ...
  const start = new Date(end);
  start.setDate(end.getDate() - (days - 1));
  const url = `${EXCHANGE_HOST}/timeseries?start_date=${fmtDate(start)}&end_date=${fmtDate(end)}&base=TRY&symbols=USD,EUR`;
  const res = await fetch(url);
  if (!res.ok) throw new Error("exchangerate.host request failed");
  const data = await res.json();
  if (!data || !data.rates) throw new Error("exchangerate.host invalid payload");
